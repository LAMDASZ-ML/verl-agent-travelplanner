...
class GoogleDistanceMatrix:
    ...
    def __init__(
        ...
        path="../database/googleDistanceMatrix/distance.csv",
    ) -> None:
        self.gplaces_api_key: str = subscription_key
        if not self._inited:
            self._inited = True
            self.__class__.data = pd.read_csv(path)
    ...
    def run(self, origin, destination, mode='driving'):
        ...
        if len(response) > 0:
            ...
            if "driving" in mode:
                info["cost"] = int(
                    eval(info["distance"].replace("km", "").replace(",", "")) * 0.05
                )
            elif mode == "taxi":
                info["cost"] = int(
                    eval(info["distance"].replace("km", "").replace(",", ""))
                )
